...
請你將這些筆記內容進行深度邏輯整合，撰寫成一份**有血有肉、內容紮實**的工作匯報。

## 核心原則（必須嚴格遵守）

1. **必須引用原文**：在每個重點段落中，你必須直接引用使用者筆記中的具體字句、事件名稱、任務項目或數字資料（用引號「」標示），不得憑空補充、臆測或使用與筆記無關的套話。
2. **時間脈絡分析**：依照記錄時間順序，分析工作進度的演變與推進節奏。若同一任務在不同時間點都有記錄，請明確描述其前後變化（例如「X 從 [時間A] 提出後，於 [時間B] 完成」）。
3. **具體待辦識別**：只列出筆記中**明確提及或強烈隱含**的待辦事項，並標明出處（哪條筆記、什麼時間提及）。禁止憑空生成與筆記無關的「建議」。
4. **助理反饋有據可查**：「助理提醒」必須基於可觀察的事實，例如：記錄集中在哪個時段、是否有重複出現的關鍵詞、筆記中是否流露焦慮或緊迫感等。不得使用「希望你注意休息」等無依據的套話。
5. **禁止空洞總結**：每個章節的第一句話必須是具體的陳述或引用，而非「本次筆記記錄了...」這類無資訊量的開場白。
...
- # [筆記本標題] - 工作整合匯報
- ## 📋 核心進展概述
  （2-4 句話精準總結，必須包含筆記中最重要的任務名稱或事件，附上時間節點）
- ## 🔍 分類重點與進度詳析
  （依任務類別或主題分組，每項都要引用筆記原文片段，描述具體狀態：已完成 / 進行中 / 待處理）
- ## 🚀 明確待辦清單
  （僅列筆記中有依據的待辦項目，格式：[ ] 任務名稱 — 依據：「原文引用」）
- ## 💡 助理觀察與提醒
  （基於筆記的時間分佈、頻率、語氣等具體觀察，給出有針對性的反饋；若無明顯觀察則省略此章節）

使用專業且親切的商務語氣。`;

        const userPrompt = `筆記本名稱：${notebook.title}
建立時間：${notebook.created_at}
筆記條數：${notebookEntries.length} 條
...
// --- Local Data Backup & Restore Logic ---
function exportBackup() {
    try {
        ...
        const fileName = `will_ai_backup_${yyyy}${mm}${dd}_${hh}${min}${ss}.json`;
...
function importBackup(file) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (!Array.isArray(data.notebooks) || !Array.isArray(data.entries)) {
                alert("匯入失敗：備份檔案中缺乏核心筆記資料。");
                return;
            }

            const confirmMsg = "匯入備份將會覆蓋您目前的所有本機筆記、主題分類與設定，此動作無法復原。\n確定要進行匯入與覆蓋嗎？";
            if (!confirm(confirmMsg)) return;

            // Save elements to localStorage
            localStorage.setItem(STORAGE_KEYS.NOTEBOOKS, JSON.stringify(data.notebooks));
            ...
            localStorage.setItem(STORAGE_KEYS.MODEL, data.model || 'gemini-2.5-flash');
            localStorage.setItem(STORAGE_KEYS.TRASH_NOTEBOOKS, JSON.stringify(data.trashNotebooks || []));
            localStorage.setItem(STORAGE_KEYS.TRASH_ENTRIES, JSON.stringify(data.trashEntries || []));
            localStorage.setItem(STORAGE_KEYS.THEME, data.theme || 'classic');
            ...
            alert("備份資料已匯入成功！網頁即將重新載入以套用資料。");
            location.reload();
        } catch (err) {
            console.error("Backup import failed:", err);
            alert("匯入失敗：無法解析該備份 JSON 檔案。");
        }
    };
    reader.readAsText(file);
}

// Bind Backup UI elements on DOMContentLoaded
    ...
    if (backupFileInput) {
        backupFileInput.addEventListener('change', (e) => {
            if (e.target.files.length > 0) {
                ...
                // Clear input so user can import the same file again if needed
                e.target.value = '';
            }
        });
    }
...
if (saveSettingsBtn) {
    saveSettingsBtn.addEventListener('click', async () => {
        ...
        if (settingsStatus) {
            ...
            settingsStatus.textContent = '設定儲存成功！';
        }
